...
SCOPE = "user-modify-playback-state user-read-currently-playing playlist-modify-private playlist-modify-public playlist-read-private"
...
if __name__ == '__main__':
    ...
    a = []
    # kkk = open("tt.txt","a+")
    # for i in range(0, spotifyObject_1.user_playlist_tracks(playlist_id="2xmYsScxxyyBmg3FCftaZJ")["total"]):
    #     # a.append(spotifyObject_1.user_playlist_tracks(offset=i, limit=1, playlist_id="4QGhaoFzUW8m7qeBur7Kfi")["items"][0]["track"]["id"])
  
    #     kkk.write(spotifyObject_1.user_playlist_tracks(offset=i, limit=1, playlist_id="2xmYsScxxyyBmg3FCftaZJ")["items"][0]["track"]["id"]+"\n")
    # kkk.close()

    for s in range(0,spotifyObject_1.user_playlists(user=USERNAME)["total"]):
        a.append(spotifyObject_1.user_playlists(user=USERNAME)["items"][s]["name"])
    if "my super cool playlist" not in a:
        spotifyObject_1.user_playlist_create(user=USERNAME, name="my super cool playlist", public=False, description="my super cool playlist made with my super cool program")
        for s in range(0,spotifyObject_1.user_playlists(user=USERNAME)["total"]):
            if spotifyObject_1.user_playlists(user=USERNAME)["items"][s]["name"] == "my super cool playlist":
